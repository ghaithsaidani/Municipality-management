...
public class AjouterIntervention_Controller implements Initializable {
    @FXML
    ...
    void ajouter(ActionEvent event) {
        ...
        try {
            if (Validate.verifierchaine(nom.getText()) && Validate.verifierchaine(prenom.getText()) && Validate.verifierId(cin.getText()) && Validate.verifierTel(tel.getText()) && Validate.verifierchaine(cat.getText()) && Validate.verifierchaine(sujet.getText()) && date_reclamation.getValue()!=null) {
                ...
                l.setBody(new Text("Voulez vous créer cet Reclamation ?"));
                ...
                b0.addEventHandler(MouseEvent.MOUSE_CLICKED, (MouseEvent ev) ->
                {
                    ...
                    try {
                        Intervention r = new Intervention(nom.getText(), prenom.getText(), Integer.parseInt(cin.getText()), Integer.parseInt(tel.getText()), cat.getText(), sujet.getText(), date_reclamation.getValue(),desc.getText());
                        PreparedStatement pre = con.prepareStatement("insert into reclamation values(?,?,?,?,?,?,?,to_date(?,'YYYY-MM-DD'),?)");
                        pre.setInt(1, r.getNum());
                        pre.setString(2, r.getNom());
                        pre.setString(3, r.getPrenom());
                        pre.setInt(4, r.getCin());
                        pre.setInt(5, r.getTel());
                        pre.setString(6, r.getCategorie());
                        pre.setString(7, r.getSujet());
                        pre.setString(8, r.getDate_reclamation().toString());
                        pre.setString(9, r.getDescription());
                        pre.executeUpdate();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    ...
                    tray.setImage(i);
                    tray.setTitle("AJOUT DE LA RECLAMATION AVEC SUCCÉS");
