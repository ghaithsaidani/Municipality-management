...
public class Modifier_Revenu_Controller implements  Initializable{
    int n;
    @FXML
    ...
    void modifier() {
        ...
        try {
            ...
            if (Validate.verifierchaine(rev.getText())) {
                ...
                l.setBody(new Text("Voulez vous modifier cette Revenu ?"));
...

                {
                    ...
                    try {
                        Revenu r =new Revenu(Float.parseFloat(montant.getText()) ,Float.parseFloat(rev.getText()),date.getValue(),description.getText());
                        PreparedStatement pre = DbConnector.getConnection().prepareStatement("update revenu set montant=?,rev=?,description=?,date=to_date(?,'YYYY-MM-DD'),where num=?");
                        pre.setFloat(1, r.getMontant());
                        pre.setFloat(2, r.getRev());
                        pre.setString(3, r.getDescription());
                        pre.setString(4, r.getDate().toString());
                        pre.setInt(5,r.getNum());
                        pre.execute();
                    } catch (Exception e) {
