...
public class ModifierCompte_Controller implements Initializable {
    ...
    public void showinfo(String s1,String s2,String s3,String s4,String s5)
    {
        username.setText(s1);
        mail.setText(s2);
        poste.setValue(s3);
    ...
    void modify(ActionEvent event) {
        try{
            ...
            if(Validate.verifierpassword(password.getText()) && !password.getText().equals(confirm_password.getText()))
            {
            ...
            else if (Validate.verifierId(username.getText()) && Validate.verifieremail(mail.getText()) && poste.getValue()!=null && Validate.verifierpassword(password.getText()) && password.getText().equals(confirm_password.getText())) {
                JFXButton b0 = new JFXButton("OUI");
                JFXButton b1 = new JFXButton("NON");
                b0.setId("jfxbutton");
                b1.setId("jfxbutton");
                JFXDialogLayout l = new JFXDialogLayout();
                l.setBody(new Text("Voulez vous modifier ce compte ?"));
                ...
                b0.addEventHandler(MouseEvent.MOUSE_CLICKED, (MouseEvent ev) ->
                {
                    ...
                    try {
                        ...
                        PreparedStatement pre = con.prepareStatement("update comptes set e_mail=?,poste=?,password=? where id=?");
                        pre.setString(1, c.getE_mail());
                        pre.setString(2, c.getPoste());
                        pre.setString(3, c.getPassword());
                        pre.setInt(4, c.getEmp().getId());
                        pre.execute();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
